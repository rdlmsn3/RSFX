...
EventBus integration:
    TradeEngine subscribes to SignalEvent. When SignalEngine publishes
    a SignalEvent, queue_order() is called automatically.
...
Usage:
    ...
    # Manual:
    engine.queue_order(signal)

    # Or via EventBus (SignalEngine publishes SignalEvent → auto queue_order):
    # ... just call engine.on_tick() in the tick loop

    for ts, bid, ask, vol in tick_stream:
        engine.on_tick(bid, ask, ts)          # manage open pos + fill queue
        candle_stream.ingest_tick(ts, bid, ask, vol)  # EventBus handles signals
"""
...
class TradeEngine:
    ...
    EventBus integration:
      Subscribes to SignalEvent — when SignalEngine publishes a signal,
      queue_order() is called automatically.
    ...
    def __init__(self, config: TradeConfig, event_bus: EventBus | None = None) -> None:
        ...
        # EventBus: subscribe to SignalEvent for automatic queue_order()
        if self._bus:
