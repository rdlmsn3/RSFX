"""
detectors/strategies/volume_divergence.py
-----------------------------------------
OBV divergence from price (M5 only).

Rules:
  LONG:  price makes lower low + OBV makes higher low  (bullish divergence)
  SHORT: price makes higher high + OBV makes lower high (bearish divergence)
...
try:
    ...
    TA_AVAILABLE = False
    logger.warning("TA-Lib not installed. Volume Divergence signals disabled.")


class VolumeDivergenceStrategy(BaseStrategy):
    """OBV divergence from price on M5."""
    ...
    def evaluate(
        ...
        obv_lows = self._find_swing_lows(obv_lb)

        if len(price_lows) >= 2 and len(obv_lows) >= 2:
            pl1, pl2 = price_lows[-2], price_lows[-1]
            ol1, ol2 = obv_lows[-2], obv_lows[-1]
            if pl2[1] < pl1[1] and ol2[1] > ol1[1]:
                detected.append(PatternSignal(
                    ...
                    metadata={
                        ...
                        "obv_low1": float(ol1[1]),
                        "obv_low2": float(ol2[1]),
                    },
                ))
        ...
        obv_highs = self._find_swing_highs(obv_lb)

        if len(price_highs) >= 2 and len(obv_highs) >= 2:
            ph1, ph2 = price_highs[-2], price_highs[-1]
            oh1, oh2 = obv_highs[-2], obv_highs[-1]
            if ph2[1] > ph1[1] and oh2[1] < oh1[1]:
                detected.append(PatternSignal(
                    ...
                    metadata={
                        ...
                        "obv_high1": float(oh1[1]),
                        "obv_high2": float(oh2[1]),
                    },
                ))
